...
<script>
    ...
    var alldriver=database.collection('owner_users')
                ...
                documents.forEach((ele) => {
                    ...
                    docRefs.get().then(async function(docrefSnapshot) {
                       ...
                       var docRef = docrefSnapshot.data() && docrefSnapshot.data().documents
                        ? docrefSnapshot.data().documents.find(d => d.documentId == doc.id)
                        : null;
                        console.log(docRef);
                        ...
                        if (docRef) {
                            if (docRef.verified === true && docRef.status === "Approved") {
                                status = "Approved";
                            } else if (docRef.verified === false && docRef.status === "DisApproved") {
                                status = "DisApproved";
                            } else {
                                status = "Not Approved";
                            }
            ...
            database.collection('driver_document').doc(id)
                .get().then((doc) => {
                    ...
                    database.collection('driver_document').doc(id).update({
                        ...
                        await ref.get().then(async function(snapshotsdriver) {
