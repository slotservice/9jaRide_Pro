                    ...
                    <div class="row restaurant_payout_create">
                        <div class="restaurant_payout_create-inner">
                            <fieldset>
                                ...
                                <div class="form-group row width-50">
                                    <div class="form-check">
                                        <input type="checkbox" class="is_default" id="is_default">
                                        <label class="col-3 control-label" for="is_default">{{trans('lang.is_default')}}</label>
                                    </div>
                                </div>
                                <div class="form-group row width-50">
                                    <div class="form-check">
...
@section('scripts')
    <script>
        ...
        var requestId = "{{$id}}";
        var flagImageFile = '';
        var defaultLangAlert="{{trans('lang.default_lang_alert')}}";
                ...
                window.scrollTo(0, 0);
                 ...
                 if(is_default) {
                    await database.collection('languages').where('isDefault',"==",true).get().then(async function(snapshots) {
                        var defaultLang=snapshots.docs[0].data();
                        var defaultLangId=defaultLang.id;
                        await database.collection('languages').doc(defaultLangId).update({'isDefault': false});
                    });
                }else{
                        await database.collection('languages').where('isDefault',"==",true).get().then(function(snapshots) {
                         if(snapshots.docs.length==0) {
                             alert(defaultLangAlert);
                             $(".is_default").prop('checked',true);
                             is_default=true;
                             return false;
                         }else if(snapshots.docs.length>0){
                             var defaultLang=snapshots.docs[0].data();
                             var defaultLangId=defaultLang.id;
                             if(snapshots.docs.length==1 && defaultLangId==requestId){
