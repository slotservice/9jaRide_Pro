    ...
    <div class="row page-titles">
        ...
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{url('/reports/driver')}}">{{trans('lang.report_plural')}}</a>
                </li>
        ...
        <div class="card  pb-4">
            ...
            <div class="card-body">
                ...
                <div class="form-group col-12 text-center btm-btn">
                    <button type="submit" class="btn btn-primary download-driver-report"><i class="fa fa-save"></i>
...
<script>
        ...
        if(serviceId) {
            await database.collection('service').where('id','==',serviceId).get().then(async function(snapshots) {
                if(snapshots.docs.length>0) {
                    ...
                    if(Array.isArray(service.title)) {
                        var foundItem=service.title.find(item => item.type===setLanguageCode);
                        if(foundItem&&foundItem.title!='') {
                            title=foundItem.title;
                        } else {
                            var foundItem=service.title.find(item => item.type===defaultLanguageCode);
                            if(foundItem&&foundItem.title!='') {
                                title=foundItem.title;
                            } else {
                                var foundItem=service.title.find(item => item.type==='en');
                                title=foundItem ? foundItem.title : '';
                            }
                        }

                    }
                    if (title === '' && Array.isArray(service.title) && service.title.length > 0) {
                        title = service.title[0].name || service.title[0].title || '';
                    }
        ...
        if(vehicleId) {
            await database.collection('vehicle_type').where('id','==',vehicleId).get().then(async function(snapshots) {
                if(snapshots.docs.length>0) {
                    ...
                    if(Array.isArray(vehicle.name)) {
                        var foundItem=vehicle.name.find(item => item.type===setLanguageCode);
                        if(foundItem&&foundItem.name!='') {
                            title=foundItem.name;
                        } else {
                            var foundItem=vehicle.name.find(item => item.type===defaultLanguageCode);
                            if(foundItem&&foundItem.name!='') {
                                title=foundItem.name;
                            } else {
                                var foundItem=vehicle.name.find(item => item.type==='en');
                                title=foundItem ? foundItem.name : '';
                            }
                        }

                    }
                    if (title === '' && Array.isArray(vehicle.name) && vehicle.name.length > 0) {
                        title = vehicle.name[0].name || vehicle.name[0].title || '';
                    }
    ...
    $(document).on('click','.download-driver-report',function() {
        ...
        var headerArray=['Name','Email','PhoneNumber','LoginType','Document Verified','Wallet Amount','latitude','longitude','Vehicle Color','Vehicle Number','Vehicle Seats','Vehicle Registration date','Vehicle Name','Service'];
        var headers=[];
    ...
    async function getData(querySnapshot) {
        ...
        await Promise.all(querySnapshot.docs.map(async (doc) => {
            ...
            newDriverObj['Email']=shortEmail(driverObj.email);
            if(driverObj.hasOwnProperty('countryCode') && driverObj.countryCode!=null && driverObj.countryCode!='' && driverObj.countryCode.includes('+')) {
                driverObj.countryCode=driverObj.countryCode.slice(1);
            }
            else {
                driverObj.countryCode=driverObj.countryCode;
            }

            var phoneNumber=shortReportNumber(driverObj.countryCode,driverObj.phoneNumber);
            newDriverObj['PhoneNumber']=phoneNumber;
