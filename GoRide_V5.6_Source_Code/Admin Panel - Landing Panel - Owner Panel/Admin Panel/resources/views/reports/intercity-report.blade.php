        ...
        async function getData(querySnapshot, fileFormat) {
            ...
            await Promise.all(querySnapshot.docs.map(async (doc) => {
                ...
                if (obj.userId) {
                    ...
                    newObj['User Email'] = userInfo && userInfo.email? shortEmail(userInfo.email):'';
                    userCountryCode = userInfo.countryCode.includes('+') ? userInfo.countryCode.slice(1) : userInfo.countryCode;
                    newObj['User Phone Number'] = userInfo && userInfo.phoneNumber ? shortReportNumber(userCountryCode, userInfo.phoneNumber) :'';
                ...
                if (obj.driverId) {
                    ...
                    newObj['Driver Email'] = driverInfo && driverInfo.email? shortEmail(driverInfo.email):'';
                    driverCountryCode = driverInfo.countryCode.includes('+') ? driverInfo.countryCode.slice(1) : driverInfo.countryCode;
                    newObj['Driver Phone Number'] = driverInfo && driverInfo.phoneNumber ? shortReportNumber(driverCountryCode, driverInfo.phoneNumber):'';
                ...
                var name='';
                if(Array.isArray(obj.intercityService.name)) {
                    var foundItem=obj.intercityService.name.find(item => item.type===setLanguageCode);
                    if(foundItem&&foundItem.name!='') {
                        name=foundItem.name;
                    } else {
                        var foundItem=obj.intercityService.name.find(item => item.type===defaultLanguageCode);
                        if(foundItem&&foundItem.name!='') {
                            name=foundItem.name;
                        } else {
