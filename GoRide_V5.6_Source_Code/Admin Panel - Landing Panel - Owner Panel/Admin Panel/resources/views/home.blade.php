            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="row">
                        ...
                        <div class="col-md-4" onclick="location.href='{!! route('users.index') !!}'">
                            <div class="card card-box-with-icon border-0">
                                <div class="card-body p-3">
                                    <span class="box-icon ab"><img src="{{ asset('images/total_user.png') }}"></span>
                                    <div class="card-box-with-content mt-3">

                                        <h4 class="card-left-title text-dark font-medium">
                                            {{trans('lang.dashboard_total_clients')}}
                                        </h4>
                                        <h3 class="m-b-0 text-dark-2 font-bold mb-2 total_earning" id="users_count">00
                                        </h3>

                                        <h6 id="users_percent" class="green up-down-list font-semibold"></h6>
                ...
                <div class="col-md-8 col-lg-8">
                    <div class="card border">
                        <div class="card-header no-border d-flex justify-content-between">
                            ...
                            <a href="{!! route('rides') !!}">{{ trans('lang.view_all') }}</a>
                        </div>
                        <div class="card-body">
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        function loadRecentRidesAndTopDrivers(filterType = 'year', year = null, month = null, startDate = null, endDate = null) {
            ...
            let driversQuery = database.collection('driver_users').orderBy('reviewsCount', 'desc').limit(limit);
        ...
        function loadDashboardData(filterType = null, year = null, month = null, startDate = null, endDate = null) {
            let now = new Date();
            ...
            Promise.all([
                ...
                database.collection('orders_intercity').get(),
                database.collection('users').get(),
                database.collection('driver_users').get(),

                // Current period
                database.collection('orders').where('createdDate', '>=', startThisTS).where('createdDate', '<=', endThisTS).get(),
                database.collection('orders_intercity').where('createdDate', '>=', startThisTS).where('createdDate', '<=', endThisTS).get(),
                database.collection('users').where('createdAt', '>=', startThisTS).where('createdAt', '<=', endThisTS).get(),
                database.collection('driver_users').where('createdAt', '>=', startThisTS).where('createdAt', '<=', endThisTS).get(),

                // Last period (skip for custom range)
                startLastTS ? database.collection('orders').where('createdDate', '>=', startLastTS).where('createdDate', '<=', endLastTS).get() : Promise.resolve({ docs: [] }),
                startLastTS ? database.collection('orders_intercity').where('createdDate', '>=', startLastTS).where('createdDate', '<=', endLastTS).get() : Promise.resolve({ docs: [] }),
                startLastTS ? database.collection('users').where('createdAt', '>=', startLastTS).where('createdAt', '<=', endLastTS).get() : Promise.resolve({ docs: [] }),
                startLastTS ? database.collection('driver_users').where('createdAt', '>=', startLastTS).where('createdAt', '<=', endLastTS).get() : Promise.resolve({ docs: [] })
            ]).then(([allOrders, allIntercity, allUsers, allDrivers, ordersCurr, intercityCurr, usersCurr, driversCurr, ordersLast, intercityLast, usersLast, driversLast]) => {

                // Totals
                let totalRides = allOrders.docs.length + allIntercity.docs.length;
                let totalUsers = allUsers.docs.length;
                ...
                let totalUsersCurr = filterType ? usersCurr.docs.length : totalUsers;
                ...
                let totalUsersLast = usersLast.docs.length;
                ...
                let usersPercent = totalUsersLast === 0
                    ? (totalUsersCurr === 0 ? 0 : 100)
                    : ((totalUsersCurr - totalUsersLast) / totalUsersLast) * 100;
                ...
                jQuery("#users_count").text(totalUsersCurr);
                jQuery("#driver_count").text(totalDriversCurr);

                if(filterType !== null){
                    jQuery("#rides_percent").html(`<i class="fa ${ridesInfo.arrow}"></i> ${Math.abs(ridesPercent).toFixed(2)}% {{trans('lang.vs_last_month')}}`).removeClass('green red').addClass(ridesInfo.className).show();
                    jQuery("#users_percent").html(`<i class="fa ${usersInfo.arrow}"></i> ${Math.abs(usersPercent).toFixed(2)}% {{trans('lang.vs_last_month')}}`).removeClass('green red').addClass(usersInfo.className).show();
                    jQuery("#driver_percent").html(`<i class="fa ${driversInfo.arrow}"></i> ${Math.abs(driversPercent).toFixed(2)}% {{trans('lang.vs_last_month')}}`).removeClass('green red').addClass(driversInfo.className).show();
                }else{
                    jQuery("#rides_percent").hide();
                    jQuery("#users_percent").hide();
        ...
        async function getTotalEarnings(filterType = null, year = null, month = null, startDate = null, endDate = null) {
            ...
            let earningsQuery = database.collection('orders').where('status', 'in', ["Ride Completed"]);
        ...
        async function getTotalEarningsIntercity(filterType = null, year = null, month = null, startDate = null, endDate = null, rides_data) {
            
            var v01 = 0, v02 = 0, v03 = 0, v04 = 0, v05 = 0, v06 = 0, v07 = 0, v08 = 0, v09 = 0, v10 = 0, v11 = 0, v12 = 0;
            
            // Build Firestore query
            let intercityQuery = database.collection('orders_intercity').where('status', '==', 'Ride Completed');
            ...
            const data = {
                ...
                datasets: [{
                    data: [
                        ...
                        parseInt(jQuery("#users_count").text()) || 0,
