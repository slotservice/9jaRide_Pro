...
<div class="page-wrapper">
    ...
    trans('lang.save')}}
                    ...
                    <a href="{{url('owners/document/' . $ownerId)}}" class="btn btn-default"><i
                            class="fa fa-undo"></i>{{
    trans('lang.cancel')}}</a>
                </div>
            </div>
    ...
    <script>
        var docId="{{$id}}";
        var id="{{$ownerId}}";
        var allowner=database.collection('owner_users').where('id','==',id);
        ...
        $(document).ready(function() {
            ...
            var ownerDocRef=database.collection('driver_document').doc(id);
            docRef.get().then(async function(Snapshot) {
                var docRef=Snapshot.data();
                ownerDocRef.get().then(async function(docrefSnapshot) {
                    var ownerDocRef=docrefSnapshot.data()&&docrefSnapshot.data().documents? docrefSnapshot.data().documents.filter((doc) => doc.documentId.trim()==docId.trim())[0]:[];
                    ...
                    if(docRef.enable) {
                        var title='';
                        if(Array.isArray(docRef.title)) {
                            var foundItem=docRef.title.find(item => item.type===setLanguageCode);
                            if(foundItem&&foundItem.title!='') {
                                title=foundItem.title;
                            } else {
                                var foundItem=docRef.title.find(item => item.type===defaultLanguageCode);
                                if(foundItem&&foundItem.title!='') {
                                    title=foundItem.title;
                                } else {
                        ...
                        html+='<label class="col-3 control-label">'+"{{trans('lang.document_number')}}"+'<span class="required-field"></span></label><div class="col-7" class="document_number"><input type="text" class="form-control document_number" value="'+(ownerDocRef&&ownerDocRef.documentNumber? ownerDocRef.documentNumber:'')+'"></div></div>';
                        if(ownerDocRef&&ownerDocRef.expireAt) {
                            var date=new Date(ownerDocRef.expireAt.seconds*1000);
                        ...
                        html+='<input type="hidden" name="isAdd" id="isAdd" value="'+(ownerDocRef&&ownerDocRef.documentId? false:true)+'">';
                        html+='<div class="form-group row width-100"><div class="form-check"><input type="checkbox" class="document_verified" id="document_verified" '+(ownerDocRef&&ownerDocRef.verified? 'checked':'')+'><label class="col-3 control-label" for="document_verified">'+"{{trans('lang.document_verified')}}"+'</label></div></div> ';
                        html+='</fieldset>';
                    }
                    $(".doc-body").html(html);
                    jQuery("#overlay").hide();
