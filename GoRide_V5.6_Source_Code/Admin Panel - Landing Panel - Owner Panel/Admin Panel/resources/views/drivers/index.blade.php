...
@section('scripts')
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            const table = $('#driverTable').DataTable({
                ...
                ajax: async function(data, callback, settings) {
                    ...
                    const orderableColumns = (checkDeletePermission) ? ['', 'fullName', 'email', 'phone', '', 'createdAt', 'expiryDate', 'activePlanName', 'serviceName', 'totalRide', ''] : ['fullName', 'email', 'phone', '', 'createdAt', 'expiryDate', 'activePlanName', 'service', 'totalRide', '']; // Ensure this matches the actual column names
                    const orderByField = orderableColumns[orderColumnIndex]; // Adjust the index to match your table
                    if (searchValue.length >= 3 || searchValue.length === 0) {
                        $('#overlay').show();
                    }
                    ((type === 'approved' || type === 'pending') ? ref : ref.orderBy('createdAt', 'desc')).get().then(async function(querySnapshot) {
                        ...
                        await Promise.all(querySnapshot.docs.map(async (doc) => {
                            childData = doc.data();
                            childData.id = doc.id; 
                            if (childData.hasOwnProperty('ownerId') && childData.ownerId !== null) {
                                return; 
                            }  
                ...
                columnDefs: [{
                    ...
                    {
                        orderable: false,
                        targets: (checkDeletePermission) ? [0, 4, 9, 10] : [3, 8, 9]
                    },
                ],
        ...
        async function buildHTML(val) {
            ...
            html.push(userImg + '<a href="' + driverView + '">' + val.fullName + '</a>' + verified);
                ...
                jQuery.ajax({
                    ...
                    error: function (xhr, status, error) {
                        try {
                            ...
                            if (responseText.error && responseText.error.includes("NOT_FOUND")) {
        ...
        async function countUnreadMessages(driverId) {
            var unreadCount = 0;
            await database.collection('chat_admin').doc(driverId).collection("thread")
                .where("seen", "==", false)
                .onSnapshot(snapshot => {
                    // Filter out admin messages in code so the query needs no composite index.
