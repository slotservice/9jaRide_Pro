...
class CompleteOrderController extends GetxController {
  ...
  Future<void> calculateAmount() async {
    ...
    if (orderModel.value.driverId != null && orderModel.value.driverId!.isNotEmpty) {
      ...
      nonAcChargeValue = double.parse(orderModel.value.service?.firstPrice.nonAcCharge ?? '0.0');
      acChargeValue = double.parse(orderModel.value.service?.firstPrice.acCharge ?? '0.0');
      kmCharge = double.parse(orderModel.value.service?.firstPrice.kmCharge ?? '0.0');
    }

    totalChargeOfMinute.value = double.parse(durationValueInMinutes.toString()) * double.parse(orderModel.value.service?.firstPrice.perMinuteCharge ?? '0.');
    ...
    // Minimum fare: no ride is charged less than this. Must stay identical to
    // the customer app or the two would disagree on the total.
    ...
    // Special assistance is charged ON TOP of the minimum, never absorbed by
    // it, so the driver genuinely earns the extra for taking that rider.
