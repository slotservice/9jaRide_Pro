...
class FireStoreUtils {
  ...
  static Future<List<DocumentModel>> getDocumentList() async {
    ...
    await fireStore.collection(CollectionName.documents).where('enable', isEqualTo: true).where('type', isEqualTo: Constant.currentUserType).where('isDeleted', isEqualTo: false).get().then((value) {
  ...
  static StreamController<List<OrderModel>>? getNearestOrderRequestController;

  static Stream<List<OrderModel>> getOrders(DriverUserModel driverUserModel, double? latitude, double? longLatitude) async* {
  ...
  static StreamController<List<InterCityOrderModel>>? getNearestFreightOrderRequestController;

  static Stream<List<InterCityOrderModel>> getFreightOrders(double? latitude, double? longLatitude) async* {
  ...
  static Future<bool?> acceptInterCityRide(InterCityOrderModel orderModel, DriverIdAcceptReject driverIdAcceptReject) async {
    bool isAdded = false;
    await fireStore.collection(CollectionName.ordersIntercity).doc(orderModel.id).collection("acceptedDriver").doc(driverIdAcceptReject.driverId).set(driverIdAcceptReject.toJson()).then((value) {
      isAdded = true;
    }).catchError((error) {
      log("Failed to update user: $error");
      isAdded = false;
  ...
  static Future<bool> deleteUser() async {
    try {
      ...
      await fireStore.collection(CollectionName.driverUsers).doc(userId).delete();
      ...
      if (referralModel!.referralBy != null && referralModel!.referralBy!.isNotEmpty) {
        await fireStore.collection(CollectionName.driverUsers).doc(referralModel!.referralBy).get().then((value) async {
          ...
          if (userDocumentDriver.data() != null && userDocumentDriver.exists) {
            try {
              DriverUserModel driver = DriverUserModel.fromJson(userDocumentDriver.data()!);
              double referralAmount = double.parse(Constant.referralDriverAmount ?? '0.0');
              bool? isUpdateBalance = await updatedDriverWalletById(driverId: driver.id!, referralAmount: referralAmount);
        ...
        await fireStore.collection(CollectionName.users).doc(referralModel!.referralBy).get().then((value) async {
          ...
          if (userDocumentCustomer.data() != null && userDocumentCustomer.exists) {
            try {
              ...
              bool? isUpdateBalance = await updatedCustomerWalletById(customerId: user.id!, referralAmount: referralAmount);
        ...
        await fireStore.collection(CollectionName.users).doc(referralModel!.referralBy).get().then((value) async {
          ...
          if (userDocumentCustomer.data() != null && userDocumentCustomer.exists) {
            try {
              ...
              referralAmount = double.parse(Constant.referralCustomerAmount ?? '0.0');

              user.walletAmount = (double.parse(user.walletAmount ?? '0.0') + referralAmount).toString();
  ...
  static Future<bool> getCustomerFirstOrderOrNot({required String customerId, required String orderType}) async {
    try {
      final normalOrders = await fireStore.collection(CollectionName.orders).where('userId', isEqualTo: customerId).where('status', isEqualTo: Constant.rideComplete).get();
  ...
  static Future<bool> getDriverFirstOrderOrNot({required String driverId, required String orderType}) async {
    try {
      final normalOrders = await fireStore.collection(CollectionName.orders).where('driverId', isEqualTo: driverId).where('status', isEqualTo: Constant.rideComplete).get();
