...
class OrderMapController extends GetxController {
  ...
  Future<void> calculateAmount() async {
    ...
    totalChargeOfMinute.value = double.parse(durationValueInMinutes.toString()) * double.parse(orderModel.value.service?.prices?.first.perMinuteCharge ?? '1.0');
    ...
    if (distance <= double.parse(orderModel.value.service?.prices?.first.basicFare ?? '0.0')) {
      if (currentTime.isAfter(startNightTimeString) && currentTime.isBefore(endNightTimeString)) {
        amount.value = amount.value * double.parse(orderModel.value.service?.prices?.first.nightCharge ?? '1.0');
      } else {
      ...
      double basicFareValue = double.tryParse(orderModel.value.service!.prices!.first.basicFare.toString()) ?? 0.0;
      ...
      if (currentTime.isAfter(startNightTimeString) && currentTime.isBefore(endNightTimeString)) {
        amount.value = amount.value * double.parse(orderModel.value.service?.prices?.first.nightCharge ?? '1.0');
        totalChargeOfMinute.value = totalChargeOfMinute.value * double.parse(orderModel.value.service?.prices?.first.nightCharge ?? '1.0');
        basicFare.value = basicFare.value * double.parse(orderModel.value.service?.prices?.first.nightCharge ?? '1.0');
      }
    }
  ...
  Future<void> fetchRoute(location.LatLng source, location.LatLng destination) async {
    ...
    if (response.statusCode == 200) {
      ...
      for (var coord in geometry) {
        final lon = double.parse(coord[0].toString());
