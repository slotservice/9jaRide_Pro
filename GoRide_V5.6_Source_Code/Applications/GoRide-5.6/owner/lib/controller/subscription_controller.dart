...
import 'package:owner/model/stripe_failed_model.dart';
...
import 'package:owner/payment/xenditModel.dart';
...
class SubscriptionController extends GetxController {
  ...
  Rx<OwnerUserModel> ownerUserModel = OwnerUserModel().obs;
  ...
  Future<void> getInitPlanSettings() async {
    ownerUserModel.value = await FireStoreUtils.getOwnerProfile(FireStoreUtils.getCurrentUid()) ?? OwnerUserModel();
  ...
  Future<void> getSubscriptionPlanList() async {
    ...
    if (ownerUserModel.value.subscriptionPlanId == null) {
  ...
  Future createStripeIntent({required String amount}) async {
    try {
      Map<String, dynamic> body = {
        ...
        "shipping[name]": ownerUserModel.value.fullName,
  ...
  mercadoPagoMakePayment({required BuildContext context, required String amount}) async {
    ...
    final body = jsonEncode({
      ...
      "payer": {"email": ownerUserModel.value.email},
      "back_urls": {
  ...
  Future<void> payStackPayment(String totalAmount) async {
    await PayStackURLGen.payStackURLGen(
            amount: (double.parse(totalAmount) * 100).toString(), currency: "ZAR", secretKey: paymentModel.value.payStack!.secretKey.toString(), userModel: ownerUserModel.value)
        .then((value) async {
  ...
  flutterWaveInitiatePayment({required BuildContext context, required String amount}) async {
    ...
    final body = jsonEncode({
      ...
      "customer": {
        "email": ownerUserModel.value.email.toString(),
        "phonenumber": ownerUserModel.value.phoneNumber, // Add a real phone number
        "name": ownerUserModel.value.fullName!, // Add a real customer name
      },
      "customizations": {
  ...
  payFastPayment({required BuildContext context, required String amount}) {
    PayStackURLGen.getPayHTML(payFastSettingData: paymentModel.value.payfast!, amount: amount.toString(), userModel: ownerUserModel.value).then((String? value) async {
  ...
  void openCheckout({required amount, required orderId}) async {
    var options = {
      ...
      'send_sms_hash': true,
      'prefill': {
        'contact': ownerUserModel.value.phoneNumber,
        'email': ownerUserModel.value.email,
      },
      'external': {
        'wallets': ['paytm']
      }
