...
import 'package:owner/widget/osm_map/place_model.dart';
...
class OSMMapController extends GetxController {
  ...
  void selectSearchResult(Map<String, dynamic> place) {
    ...
    final city = place['address']['city'] ?? place['address']['town'] ?? place['address']['village'] ?? place['address']['state_district'] ?? place['display_name'];
  ...
  void addLatLngOnly(LatLng coords) async {
    ...
    final city = address['address']['city'] ?? address['address']['town'] ?? address['address']['village'] ?? address['address']['state_district'] ?? address['display_name'];
    pickedPlace.value = PlaceModel(coordinates: coords, address: address['display_name'], city: city);
  }
