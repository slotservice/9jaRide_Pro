...
import 'package:owner/controller/subscription_controller.dart';
...
class SubscriptionListScreen extends StatelessWidget {
  ...
  Future paymentMethodDialog(BuildContext context, SubscriptionController controller) {
                      ...
                      ButtonThem.buildButton(context, title: "Pay Now".tr, onPress: () {
                        if (controller.selectedPaymentMethod.value == '') {
                          ...
                          if (controller.selectedPaymentMethod.value == controller.paymentModel.value.wallet!.name) {
                            if (double.parse(controller.ownerUserModel.value.walletAmount.toString()) >= controller.totalAmount.value) {
                              Get.back();
                              controller.placeOrder();
                            } else {
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        controller.ownerUserModel.value.subscriptionPlanId == subscriptionPlanModel.id
                        ...
                        style: TextStyle(
                            ...
                            color: themeChange.getThem()
                                ...
                                : controller.selectedSubscriptionPlan.value.id == subscriptionPlanModel.id
                                    ...
                                    : AppColors.grey900)),
                    const SizedBox(height: 10),
                    Text('${'Add Driver limits :'.tr} ${subscriptionPlanModel.driverLimit == '-1' ? 'Unlimited'.tr : subscriptionPlanModel.driverLimit ?? '0'}',
                        textAlign: TextAlign.end,
                        maxLines: 2,
                        style: TextStyle(
                            fontSize: 14,
                    ...
                    ButtonThem.buildButton(
                      context,
                      title: controller.ownerUserModel.value.subscriptionPlanId == subscriptionPlanModel.id
                          ? "Renew".tr
                          : controller.selectedSubscriptionPlan.value.id == subscriptionPlanModel.id
                              ? "Active".tr
                              : "Select Plan".tr,
